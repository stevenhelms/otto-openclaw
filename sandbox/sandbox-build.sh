...
source "$ROOT_DIR/openclaw-lib/docker-build.sh"
...
PACKAGES="${PACKAGES:-curl wget jq coreutils grep nodejs npm python3 python3-pip python3-venv git ca-certificates golang-go rustc cargo unzip pkg-config libasound2-dev build-essential file gnupg dnsutils}"
...
if ! docker image inspect "${BASE_IMAGE}" >/dev/null 2>&1; then
  ...
  echo "Building base image via openclaw-lib/sandbox-setup.sh..."
  "$ROOT_DIR/openclaw-lib/sandbox-setup.sh"
fi

echo "Building ${TARGET_IMAGE} with system packages: ${PACKAGES}"
echo "Installing Python libraries: ${PYTHON_PACKAGES}"
